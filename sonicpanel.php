 ...
 */
class Sonicpanel extends Module
{
    /**
    ...
    public function __construct()
    {
        ...
        Language::loadLang('sonicpanel', null, dirname(__FILE__) . DS . 'language' . DS);
    ...
    {
        return [
            'tabStats' => Language::_('Sonicpanel.tab_stats', true)
        ];
    }
    ...
    {
        return [
            'tabClientActions' => Language::_('Sonicpanel.tab_client_actions', true),
            'tabClientStats' => Language::_('Sonicpanel.tab_stats', true)
        ];
    }
    ...
    {
        return ['first' => Language::_('Sonicpanel.order_options.first', true)];
    }
    ...
    public function getPackageFields($vars = null)
    {
        ...
        $package = $fields->label(Language::_('Sonicpanel.package_fields.package', true), 'package');
    ...
    private function getPackagesList($vars=array())
    {
		...
		$result[Language::_('Sonicpanel.service_field.module_configoptionmessage', true)] = Language::_('Sonicpanel.service_field.module_configoptionmessage', true);
		
		if(empty($result)) {
			return [
				Language::_('Sonicpanel.service_field.module_package_error', true) => Language::_('Sonicpanel.service_field.module_package_error', true)
			];
		}
		else {
			return $result;
    ...
    public function manageModule($module, array &$vars)
    {
        ...
        $this->view->setDefaultView('components' . DS . 'modules' . DS . 'sonicpanel' . DS);
    ...
    public function getAdminAddFields($package, $vars = null)
    {
		...
		$radiousername = $fields->label(Language::_('Sonicpanel.service_field.radiousername', true), 'radiousername');
		...
		$radiopassword = $fields->label(Language::_('Sonicpanel.service_field.radiopassword', true), 'radiopassword');
    ...
    public function getAdminEditFields($package, $vars = null)
    {
        ...
        $radiousername->attach($fields->tooltip(Language::_('Sonicpanel.service_field.radiousername.tooltip', true)));
        $fields->setField($radiousername);

        $radiopassword = $fields->label(
            Language::_('Sonicpanel.service_field.radiopassword', true),
            'radiopassword'
        );
        $radiopassword->attach($fields->fieldPassword('radiopassword', ['id' => 'radiopassword']));
        $radiopassword->attach($fields->tooltip(Language::_('Sonicpanel.service_field.radiopassword.tooltip', true)));
        $fields->setField($radiopassword);
        ...
        $rules = [
            'radiousername' => [
                'empty' => [
                    ...
                    'rule' => 'isEmpty',
                    'negate' => true,
                    'message' => Language::_('Sonicpanel.!error.radiousername.empty', true)
                ]
            ],
        ];
    ...
    public function addService(
        ...
        if (!$row) {
            $this->Input->setErrors(
                ['module_row' => ['missing' => Language::_('Sonicpanel.!error.module_row.missing', true)]]
            );
    ...
    public function suspendService($package, $service, $parent_package = null, $parent_service = null)
    {
        if (($row = $this->getModuleRow())) {
            ...
            if (!$response['status']) {
                $this->Input->setErrors(['api' => ['error' => '[Suspend] '.Language::_('Sonicpanel.!error.api.internal', true)]]);
            }
        }
    ...
    public function unsuspendService($package, $service, $parent_package = null, $parent_service = null)
    {
        if (($row = $this->getModuleRow())) {
            ...
            // If the action fails then set an error
            if (!$response['status']) {
                $this->Input->setErrors(['api' => ['error' => '[Unsuspend] '.Language::_('Sonicpanel.!error.api.internal', true)]]);
            }
        }
    ...
    public function cancelService($package, $service, $parent_package = null, $parent_service = null)
    {
        if (($row = $this->getModuleRow())) {
            ...
            // If the action fails then set an error
            if (!$response['status']) {
                $this->Input->setErrors(['api' => ['error' => '[Cancel] '.Language::_('Sonicpanel.!error.api.internal', true)]]);
            }
        }
    ...
    private function parseResponse($response)
    {
        ...
        if ($response['status'] == false) {
            $this->Input->setErrors(['api' => ['error' => '[Parse] '.Language::_('Sonicpanel.!error.api.internal', true)]]);
            $success = false;
        }
    ...
    {
        Loader::load(dirname(__FILE__) . DS . 'api' . DS . 'sonicpanel_api.php');

        return new SonicpanelApi($adminapikey, $ipaddress, $usessl, $adminusername, $useproxy);
    }
        ...
        $rules = [
		    'server_name' => [
                'valid' => [
                    'rule' => 'isEmpty',
                    'negate' => true,
                    'message' => Language::_('Sonicpanel.!error.server_name_valid', true)
                ]
            ],
			'ipaddress' => [
                'valid' => [
                    'rule' => [[$this, 'validateIP']],
                    'message' => Language::_('Sonicpanel.!error.ipaddress_valid', true)
                ]
            ],
			'hostname' => [
                'valid' => [
                    'rule' => [[$this, 'validateHostName']],
                    'message' => Language::_('Sonicpanel.!error.hostname_valid', true)
                ]
            ],
            'adminusername' => [
                'valid' => [
                    'rule' => 'isEmpty',
                    'negate' => true,
                    'message' => Language::_('Sonicpanel.!error.adminusername_valid', true)
                ]
            ],
            'adminapikey' => [
                'valid' => [
                    'rule' => 'isEmpty',
                    'negate' => true,
                    'message' => Language::_('Sonicpanel.!error.adminapikey_valid', true)
                ],
                'valid_connection' => [
                    'rule' => [
                    ...
                    'message' => '[Validate Connection] '.Language::_('Sonicpanel.!error.api.internal', true)
                ]
            ]
        ];
        ...
        $rules = [
			'meta[package]' => [
                'valid' => [
                    'rule' => 'isEmpty',
                    'negate' => true,
                    'message' => Language::_('Sonicpanel.!error.meta[package].valid', true)
                ]
            ],
        ];
